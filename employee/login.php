    ...
    if (!empty($email) && !empty($pass)) {
        ...
        if (mysqli_num_rows($result) > 0) {
            while ($rows = mysqli_fetch_assoc($result)) {
                // Check if employee status is 'deactive'
                if ($rows['status'] == 'deactive') {
                    $login_Err = "<div class='alert alert-warning alert-dismissible fade show'>
                        <strong>Your account is deactive. Please contact admin.</strong>
                            ...
                            <div class="card login-form mb-0 bgconatiner">
                                <div class="card-body pt-3 shadow ">
                                    ...
                                    <form method="POST" action=" <?php htmlspecialchars($_SERVER['PHP_SELF']) ?>">
                                        ...
                                        <div class="form-group">
                                            <input type="submit" value="Log-In" class="btn btn-primary btn-lg w-100 <?php echo ($rows['status'] == 'deactive') ? 'disabled' : ''; ?>" name="signin" >
                                        </div>
