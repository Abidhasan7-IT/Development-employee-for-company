...
<?php 
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        if(!empty($old_pass) && !empty($new_pass) && !empty($confirm_pass) ){
            ...
            $check_old_pass = "SELECT password FROM employee WHERE email = '$_SESSION[email]' && password = '$old_pass' ";
            $result = mysqli_query($conn , $check_old_pass);
            if( mysqli_num_rows($result) > 0 ){
               
                if( $new_pass === $confirm_pass ){
                  
                    $change_pass_query = "UPDATE employee SET password = '$new_pass' WHERE email = '$_SESSION[email]' ";
                    if (mysqli_query($conn , $change_pass_query) ){
                        session_unset();
                        session_destroy();
                        echo "
                        <div class='alert alert-success alert-dismissible fade show'>
                        <strong>Password Updated successfully! Log in With New Password</strong>
